...
export default function Favorites() {
  ...
  async function fetchProducts() {
    ...
    const { data, error: fetchErr } = await supabase.from("products").select("*").in("id", favorites);
    if (fetchErr) {
  ...
  return (
    <section className="px-6 md:px-12 py-14 max-w-6xl mx-auto">
      ...
      <p className="text-base mb-10" style={{ color: INK_SOFT }}>Pieces you've saved while browsing, kept right here.</p>
      {loading ? (
        ...
        <div className="py-14 text-center border" style={{ borderColor: CREAM_DARK }}>
          ...
          <p className="text-sm mb-5" style={{ color: INK_SOFT }}>Tap the heart on any piece while browsing to save it here.</p>
              ...
              <div className="card-hover" style={{ border: `1px solid ${CREAM_DARK}`, background: CREAM }}>
                <div className="relative">
                  <Link to={`/product/${piece.id}`}>
                    {piece.image_url ? <div className="img-zoom-wrap"><FadeImage src={piece.image_url} alt={piece.name} className="aspect-[4/5] h-auto" /></div> : <div className="img-zoom-wrap"><PlaceholderImage label={piece.name} tall /></div>}
                  </Link>
                  <button onClick={() => removeFavorite(piece.id)} aria-label={`Remove ${piece.name} from favorites`} className="absolute top-3 right-3 p-2" style={{ background: CREAM }}>
                    <Heart size={17} fill={ROSE} color={ROSE} />
                  </button>
                </div>
                <div className="p-4">
                  <Link to={`/product/${piece.id}`} className="block">
                    <p className="eyebrow mb-1" style={{ color: SAGE_DARK }}>{CATEGORIES.find((c) => c.id === piece.category)?.name}</p>
                    <h3 className="display text-base mb-1">{piece.name}</h3>
                  </Link>
                  <p className="text-xs mb-3" style={{ color: INK_SOFT }}>{piece.price}</p>
                  <Link to={`/enquiry?category=${encodeURIComponent(piece.category || "")}&piece=${encodeURIComponent(piece.name || "")}`} className="btn text-xs px-4 py-2 w-full block text-center" style={{ background: SAGE_DARK, color: CREAM }}>
