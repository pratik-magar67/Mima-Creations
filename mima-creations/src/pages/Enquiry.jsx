...
export default function Enquiry() {
  ...
  const [enquiry, setEnquiry] = useState({
    name: "",
    contact: "",
    contactMethod: "whatsapp", // "whatsapp" or "email" — controls the field's input type/keyboard
  ...
  function handleContactMethodChange(method) {
    setEnquiry((current) => ({
      ...
      // Switching to WhatsApp strips out anything that isn't a digit from whatever was already typed
      contact: method === "whatsapp" ? current.contact.replace(/\D/g, "") : current.contact,
    }));
  }
  ...
  function handleContactChange(e) {
    ...
    setEnquiry((current) => ({
      ...current,
      contact: current.contactMethod === "whatsapp" ? value.replace(/\D/g, "") : value,
    }));
  }
        ...
        <div className="grid sm:grid-cols-2 gap-5 mb-5">
          ...
          <div className="block">
            <div className="flex items-center justify-between mb-1">
              <label htmlFor="contact-input" className="text-xs" style={{ color: INK_SOFT }}>
                {enquiry.contactMethod === "whatsapp" ? "WhatsApp number" : "Email address"}
              </label>
              <div className="flex text-xs shrink-0" style={{ border: "1px solid #2B2620" }}>
                <button
                  type="button"
                  onClick={() => handleContactMethodChange("whatsapp")}
                  className="px-2 py-0.5"
                  style={{
                    background: enquiry.contactMethod === "whatsapp" ? SAGE_DARK : "transparent",
                    color: enquiry.contactMethod === "whatsapp" ? CREAM : INK_SOFT,
                  }}
                >
                  WhatsApp
                </button>
                <button
                  ...
                  className="px-2 py-0.5"
                  style={{
                    background: enquiry.contactMethod === "email" ? SAGE_DARK : "transparent",
                    color: enquiry.contactMethod === "email" ? CREAM : INK_SOFT,
                    borderLeft: "1px solid #2B2620",
                  }}
                >
                  Email
                </button>
            ...
            <input
              ...
              required
              name="contact"
              type={enquiry.contactMethod === "whatsapp" ? "tel" : "email"}
              inputMode={enquiry.contactMethod === "whatsapp" ? "numeric" : "email"}
              pattern={enquiry.contactMethod === "whatsapp" ? "[0-9]*" : undefined}
              maxLength={enquiry.contactMethod === "whatsapp" ? 15 : undefined}
              value={enquiry.contact}
              onChange={handleContactChange}
              placeholder={enquiry.contactMethod === "whatsapp" ? "e.g. 9779812345678" : "e.g. you@example.com"}
